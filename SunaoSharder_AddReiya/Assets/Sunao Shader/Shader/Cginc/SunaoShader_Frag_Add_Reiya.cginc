...
//-------------------------------------メインカラー
	...
	if (_UVAnimOtherTex) SubUV = MainUV;
	       ...
	       OUT.a        = saturate(MainTexSampleTex2D(MainUV).a * _Color.a * _Alpha);
	       OUT.a       *= lerp(1.0f , MonoColor(MainTexSampleTex2D(_AlphaMask  , SubUV).rgb) , _AlphaMaskStrength);
	#endif

	float3 Color        = MainTexSampleTex2D(MainUV).rgb;
...
//----オクルージョン
	if (_OcclusionMode == 1) Color *= lerp(1.0f , MainTexSampleTex2D(_OcclusionMap , SubUV).rgb , _OcclusionStrength);
...
//-------------------------------------シェーディング
	float3 ShadeMask    = MainTexSampleTex2D(_ShadeMask ,  SubUV).rgb * _Shade;
	float3 LightBoost   = 1.0f + (MainTexSampleTex2D(_LightMask ,  SubUV).rgb * (_LightBoost - 1.0f));
...
//----ライト反映
	...
	float3 DiffColor    = LightingCalc(Lighting , Diffuse , ShadeColor , ShadeMask);
		...
		if (_OcclusionMode == 0) SHDiffColor *= lerp(1.0f , MainTexSampleTex2D(_OcclusionMap , SubUV).rgb , _OcclusionStrength);
...
//-------------------------------------視差エミッション
	...
	if (ParallaxFlag) {
		float  Height      = (1.0f - MonoColor(MainTexSampleTex2D(_ParallaxDepthMap , IN.pduv).rgb)) * _ParallaxDepth;
...
//----マットキャップ
		       MatCapSmooth = MainTexSampleTex2D(_MatCapMask , IN.uv).a;
		       MatCapMask   = MainTexSampleTex2D(_MatCapMask , IN.uv).rgb;
...
//-------------------------------------リムライティング
	float3 RimLight = (float3)0.0f;
		...
		if (_RimLitEnable) {
			       ...
			       RimLight *= _RimLitColor.rgb * _RimLitColor.a * MainTexSampleTex2D(_RimLitMask , SubUV).rgb;
...
//----オクルージョンマスク
	if (_OcclusionMode == 2) OUT.rgb *= lerp(1.0f , MainTexSampleTex2D(_OcclusionMap , SubUV).rgb , _OcclusionStrength);
